...
Endpoints:
  GET /signals         → recent signal chunks
  GET /perception      → recent perception events
  GET /intelligence    → recent intelligence events
  GET /state-events    → recent state events
  GET /vessels         → vessel summaries
  GET /alerts          → recent alerts
  GET /health          → system health
  GET /bucket/status   → bucket sync status
  GET /stage-metrics   → pipeline stage metrics
  GET /trace/{id}      → trace lifecycle
...
# Add data_layer to path so we can import pipeline modules
...
def get_intelligence():
    ...
    for i, r in enumerate(reversed(records)):
        ...
        result.append({
            ...
            "evidence_chain": ie.get("evidence_chain", []),
        })
    return result
...
def get_stage_metrics():
    return [
        ...
        {"stage": "intelligence","total_events": 54, "events_per_sec": 16.1, "p50_latency_ms": 41, "p95_latency_ms": 110,"error_rate": 0.011, "status": "live"},
...
def run_pipeline_once():
    """Trigger one pipeline run and return the result."""
    try:
